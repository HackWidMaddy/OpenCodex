...
const NVIDIA_NIM_PROVIDER_NAME: &str = "NVIDIA NIM";
...
pub enum WireApi {
    ...
    #[default]
    Responses,
    /// The classic OpenAI-compatible Chat Completions API at `/v1/chat/completions`.
    Chat,
}
...
impl ModelProviderInfo {
    ...
    pub fn create_nvidia_nim_provider() -> ModelProviderInfo {
        let base_url = std::env::var("CODEX_NVIDIA_NIM_BASE_URL")
            .ok()
            .filter(|v| !v.trim().is_empty())
            .unwrap_or_else(|| NVIDIA_NIM_DEFAULT_BASE_URL.to_string());

        ModelProviderInfo {
            name: NVIDIA_NIM_PROVIDER_NAME.into(),
            base_url: Some(base_url),
            env_key: Some(NVIDIA_NIM_API_KEY_ENV_VAR.into()),
            env_key_instructions: Some(NVIDIA_NIM_API_KEY_INSTRUCTIONS.into()),
            experimental_bearer_token: None,
            auth: None,
            aws: None,
    ...
    pub fn is_nvidia_nim(&self) -> bool {
...
pub fn built_in_model_providers(
    ...
    // other OpenAI-compatible providers through `model_providers` in config.toml.
    [
